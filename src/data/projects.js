...
export const projects = [
  ...
  },
  {
    ...
    subtitle:
      "Does heart-disease prediction transfer across hospitals? Leave-one-hospital-out external validation, calibration, decision curves, conformal uncertainty, and a fairness audit across 4 cohorts (n=920)",
    ...
    metrics: [
      "Generalization gap: pooled-CV AUC ~0.88 to LOSO ~0.79",
      "All 3 model families indistinguishable by AUC (DeLong p > 0.33)",
    ...
    summary:
      "A clinical-ML methodology study on the UCI heart-disease cohorts (n=920) that asks the question the leaderboard ignores: does a classifier trained on some hospitals work on a hospital it has never seen? The answer is sobering and the point of the project: pooled cross-validation (AUC ~0.88) substantially overstates real transfer (leave-one-hospital-out AUC ~0.79), probability calibration breaks under cross-site label shift, the three model families are statistically indistinguishable, and a 7-feature clinical baseline ties the full model. Every claim carries a confidence interval and, where comparative, a statistical test.",
    bullets: [
      "Reframed a standard 3-model comparison into a multi-site external-validity study: leave-one-hospital-out validation across the Cleveland, Hungary, Switzerland, and VA cohorts shows pooled-CV AUC (~0.88) drops to ~0.79 out-of-hospital, the simplest model (logistic regression) transfers best, and one site (VA Long Beach) collapses to AUC 0.66 [0.56, 0.76].",
      "Made the numbers honest: nested cross-validation to strip tuning bias, bootstrap 95% CIs on every metric, and DeLong tests showing the three models are statistically indistinguishable (p > 0.33) and that a 7-feature clinical baseline ties the full 13-feature model (p = 0.68), so the added complexity buys nothing.",
      "Traced one root cause, label shift across sites (prevalence 36% to 94%), through calibration drift (ECE up to 0.30), a missingness probe that predicts the hospital at 85% accuracy but disease at chance within a single cohort, and a fairness audit where a global threshold misses women's disease 2.4x as often as men's, then shipped SHAP (8/8 features cardiologically correct), split-conformal abstention (90% coverage, defers on ~27%), a model card, datasheet, unit tests, and CI.",
    ],
    image: "/previews/uci-heart-disease.svg",
    headlineStats: [
      { value: "0.88 → 0.79", label: "Pooled-CV AUC vs. leave-one-hospital-out AUC: the real cross-hospital generalization gap" },
    ...
    caseStudy: {
      problem:
        "Heart-disease prediction on the UCI cohorts is one of the most over-fit benchmarks in ML, so a high accuracy number proves almost nothing. The questions that actually matter for clinical machine learning go unasked: does a model trained at one set of hospitals generalize to a new one, is it calibrated well enough to trust its probabilities, and does it improve decisions over a simple standard of care?",
      approach:
        "I treated the dataset's four hospital cohorts as a built-in multi-site experiment and rebuilt the study around external validity and clinical usefulness rather than leaderboard accuracy. Calibrated linear and tree-based models are evaluated both the usual pooled way and with leave-one-hospital-out validation, then stress-tested with calibration analysis, decision-curve net benefit, interpretability, conformal uncertainty, and a fairness audit, with every number carrying a confidence interval.",
      architecture: [
        "Aggregate the four UCI cohorts (n=920) and recode known data-quality artifacts (Switzerland cholesterol coded 0) as missing, with an imputation sensitivity analysis (median / MICE / KNN).",
        "Train scikit-learn pipelines (logistic regression, random forest, gradient boosting) under nested cross-validation so hyperparameter tuning doesn't optimistically bias the reported metrics.",
        ...
        "Assess trustworthiness with reliability curves (Brier, ECE), isotonic/Platt recalibration, decision-curve net benefit, and a parsimonious 7-feature clinical-proxy baseline.",
        "Explain predictions with SHAP, permutation importance, and partial dependence, cross-check the direction of every effect against cardiology, then add split-conformal prediction sets and a sex/age subgroup fairness audit.",
      ],
      findings: [
        "The generalization gap is real: pooled cross-validation AUC (~0.88) overstates leave-one-hospital-out AUC (~0.79), a drop of 0.058 to 0.081, and the simplest model (logistic regression) transfers best while flexible learners overfit site-specific structure.",
        "A single number hides failure: held out per site, AUC ranges from 0.84 (Hungary) down to 0.66 [0.56, 0.76] at VA Long Beach (near chance), and predicted probabilities barely move (~0.47-0.64) while true prevalence spans 0.36-0.94, so calibration collapses under label shift (ECE up to 0.30).",
        "Complexity didn't pay off: nested-CV AUCs are 0.880 / 0.875 / 0.880 with all pairwise DeLong tests non-significant (p > 0.33), and a 7-feature clinical baseline (AUC 0.881) is statistically indistinguishable from the 13-feature gradient-boosting model (0.874), DeLong p = 0.68.",
        "Equal AUC hid an equity gap: at a single 0.5 threshold the model misses disease in women 2.4x as often as in men (FNR 0.36 vs 0.15), driven by the same cross-site label shift, mirroring the real-world under-diagnosis of heart disease in women.",
        "The model is trustworthy where it should be: 8/8 salient features match cardiology priors (face validity), and split-conformal prediction hits its 90% coverage target (0.908) while abstaining on ~27% of patients, an actionable 'refer for more testing' signal a bare probability cannot give.",
      ],
      results: [
        { metric: "Generalization gap", value: "0.88 → 0.79", note: "Pooled-CV vs. leave-one-hospital-out AUC; logistic regression transfers best" },
        { metric: "Worst-site AUC", value: "0.66", note: "VA Long Beach held out [0.56, 0.76], near chance under heavy shift" },
        { metric: "Model comparison", value: "p > 0.33", note: "Nested-CV AUC 0.880 / 0.875 / 0.880; 3 families indistinguishable (DeLong)" },
        { metric: "Clinical baseline", value: "7 ≈ 13", note: "7-feature proxy 0.881 vs. full model 0.874; DeLong p = 0.68" },
        { metric: "Fairness (sex)", value: "2.4×", note: "FNR 0.36 (women) vs. 0.15 (men) at a single threshold" },
        { metric: "Conformal coverage", value: "0.908", note: "90% target met; abstains on ~27% of patients" },
        { metric: "Face validity", value: "8/8", note: "Salient features match cardiology priors (SHAP + signed coefficients)" },
      ],
      limitations: [
        "Small, decades-old data (n=920 across 4 cohorts): a methodology study, not a deployable tool, with limited subgroup power, especially for women.",
        "Switzerland and VA Long Beach have few negatives and heavy missingness, so their per-site numbers carry wide CIs and shouldn't be over-interpreted.",
        "Feature-support shift bounds transfer: the most predictive features (ca, thal, slope) are ~90-99% missing outside Cleveland, and no imputer can recover them.",
        "The clinical proxy is not a validated score (the data lacks Framingham/ASCVD inputs), and split-conformal guarantees marginal, not per-subgroup, coverage.",
      ],
      future: [
        "Group-conditional (Mondrian) conformal and group-aware, prevalence-adjusted thresholds, then re-audit the women's false-negative gap.",
        "Label-shift correction (prevalence re-weighting / domain adaptation) to recover cross-site calibration.",
        "Replicate on a prospective, multi-site cohort with consistent feature collection to remove the feature-support shift that bounds transfer here.",
      ],
    },
  },
];